 ...
 */
contract MintNvda is FunctionsClient, ConfirmedOwner, ERC20, Pausable {
    ...
    constructor(
        ...
        uint8 secretSlot
    ) FunctionsClient(functionsRouter) ConfirmedOwner(msg.sender) ERC20("Backed NVDA", "bNVDA") {
    ...
    function _mintFulFillRequest(bytes32 requestId, bytes memory response) internal {
        ...
        s_portfolioBalance = uint256(bytes32(response)) * getNvdaPrice();
    ...
    function getNvdaPrice() public returns (uint256) {
        tokenPrice.getCurrentPrice("NVDA");
    ...
    function getUsdValueOfNvda(uint256 tslaAmount) public returns (uint256) {
