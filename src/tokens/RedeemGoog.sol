...
contract RedeemGoog is FunctionsClient, ConfirmedOwner, Pausable, MintGoog, WithDrawalHandler {
    ...
    constructor(
        ...
        uint8 secretSlot
    ) MintGoog(i_subId, mintSource, functionsRouter, donId, usdcPriceFeed, secretVersion, secretSlot) {
    ...
    function sendRedeemRequest(uint256 amountGoog) external whenNotPaused returns (bytes32 requestId) {
        if (balanceOf(msg.sender) < (amountGoog * 1e18)) {
        ...
        s_requestIdToRequest[requestId] = dTslaRequest(amountGoog, msg.sender);

        // External Interactions
        _burn(msg.sender, amountGoog);
    }
